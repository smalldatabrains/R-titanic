...
```{r}
nn<-neuralnet(formula,train_reduce_na,hidden=c(3))
plot(nn)
...
```{r}
cabin_na_vec=model.matrix(~unitprice+deck,data=cabin_na)
model<-neuralnet(deckB+deckC+deckD+deckE+deckF+deckG+deckT~unitprice,data=cabin_na_vec,hidden=c(3))
plot(model)
compute(model,cabin_to_predict)
```
